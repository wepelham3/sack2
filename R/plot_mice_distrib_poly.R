...
plot_mice_distrib_poly = function(mids.object, var.to.plot) {
  ...
  plot1 <- ggplot2::ggplot(data = df.to.plot,
                           ggplot2::aes_string(x = ".imp", y = "count", fill = var.to.plot)) +
  ...
  plot2 <- ggplot2::ggplot(data = df.to.plot,
                           ggplot2::aes_string(x = ".imp", y = "prop")) +
    ggplot2::geom_point(data = dplyr::filter(df.to.plot, .imp == 0), color = "blue", size = 3) +
    ggplot2::geom_point(data = dplyr::filter(df.to.plot, .imp != 0), color = "red", size = 3) +
    ggplot2::scale_y_continuous(labels = scales::percent) +
    ggplot2::facet_grid(as.formula(paste0(". ~ ", var.to.plot)),
                        labeller = "label_both") +
    ggplot2::geom_hline(data = df.to.plot %>%
                        dplyr::filter(.imp == 0) %>%
                        dplyr::select_(.dots = c(var.to.plot, "prop")),
                        ggplot2::aes(yintercept = prop)) +
    ggplot2::labs(x = "imputation number",
                  y = "proportion",
                  title = title)
