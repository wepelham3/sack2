...
plot_mice_distrib_binary = function(mids.object, var.to.plot) {
  ...
  ggplot2::ggplot(data = df.to.plot,
                  ggplot2::aes_string(x = ".imp", y = "prop")) +
    ggplot2::geom_point(data = dplyr::filter(df.to.plot, .imp == 0), size = 4, color = "blue") +
    ggplot2::geom_point(data = dplyr::filter(df.to.plot, .imp != 0), size = 4, color = "red") +
    ggplot2::scale_y_continuous(labels = scales::percent) +
    ggplot2::geom_hline(yintercept = df.to.plot %>%
                          dplyr::filter(.imp == 0) %>%
                          .$prop, color = "blue") +
